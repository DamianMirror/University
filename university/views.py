...
@require_http_methods(["GET"])
def lessons_by_group(request, group_id=None):
    from django.db.models.functions import ExtractWeekDay

    groups = Repository.StudentGroups.get_all()

    if group_id:
        lessons = Repository.Lessons.get_all().filter(group_id=group_id).annotate(
            week_day=ExtractWeekDay('day')
        ).order_by('week_day', 'time_schedule_lesson_number')

        group = Repository.StudentGroups.get_by_id(group_id)

        if group:
            return render(request, 'lessons/lessons_by_group.html', {'group': group, 'lessons': lessons, 'groups': groups})
        return render(request, 'error.html', {'message': 'Групу не знайдено'})

    return render(request, 'lessons/lessons_by_group.html', {'groups': groups})
...
def get_all_lessons(request):
    ...
    lessons_data = [{"id": lesson.id, "teacher": lesson.teacher.id if lesson.teacher else None, "subject": lesson.subject.id if lesson.subject else None,
                     "day_of_weak": lesson.day_of_week if lesson.day_of_week else None} for lesson in lessons]
...
def get_lesson_by_id(request, lesson_id):
    ...
    if lesson:
        lesson_data = {"id": lesson.id, "teacher": lesson.teacher.id if lesson.teacher else None, "subject": lesson.subject.id if lesson.subject else None,
                       "day_of_weak": lesson.day_of_week if lesson.day_of_week else None}
...
def create_subject(request):
    data = request.data
    subject = Repository.Subjects.create(name=data.get("name"))
...
def create_user(request):
    data = request.data
    user = Repository.Users.create(name=data.get("name"), surname=data.get("surname"), email=data.get("email"), password=data.get("password"), age=data.get("age"))
